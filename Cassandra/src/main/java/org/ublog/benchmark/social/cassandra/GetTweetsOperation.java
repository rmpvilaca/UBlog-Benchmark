...
public class GetTweetsOperation implements BenchOperation, OperationListener {
	...
	public GetTweetsOperation(ComplexBenchMarkClient client, String userId,
			int start, int count) {
		this.client = client;
		this.userId = userId;
		this.start = start;
	...
	public CollectionOperation getNextDBOperation()
		...
		if (this.user == null) {
			...
			res = new GetOperation(client, SocialBenchmark.userTable, userId,
					columns);
		} else {
			int lastTweet = Integer.valueOf(this.user
					.get(UserService.LAST_TWEET));
			String min = userId
					+ "-"
					+ Utils.getTweetPadding(Math.max(1, lastTweet - this.start
							- this.count + 1));
			String max = userId
					+ "-"
					+ Utils.getTweetPadding(Math.max(1, lastTweet - this.start));
			res = new GetRangeOperation(client, SocialBenchmark.tweetsTable,
					min, max);
			finished = true;
		}
