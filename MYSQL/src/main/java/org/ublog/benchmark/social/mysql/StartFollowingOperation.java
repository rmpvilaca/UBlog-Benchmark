	...
	public CollectionOperation getNextDBOperation() throws UnsupportedEncodingException {
		...
		case UPDATE_FOLLOWERS:
			if (!this.get)
			{
				...
				res=new GetOperation(client,SocialBenchmark.userTable,this.toStartUser,columns);	
				this.get=true;
			}
			else
			{
				...
				else
				{
					...
					for (Message tweet : recentTweets) {
						...
						while (timelineIdx < this.timeLine.size() && timelineIdx < Utils.MAX_MESSAGES_IN_TIMELINE) {
							...
							timelineDate = new Long(java.util.UUID.fromString(split[1]).timestamp());
							
							if (timelineDate > date) {
								toAddToTimeline.add(new Pair<String,String>(tweet.getDate().toString(),tweet.getId()/*timelineId*/));
								timelineIdx++;
								break;
							}
					...
					res=new PutOperation(client,SocialBenchmark.friendsTimeLineTable,this.userId,toAddToTimeline,null);
					this.finish=true;
				}
			}
			break;
